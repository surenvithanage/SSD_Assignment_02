...
if(isset($accessToken)){
	...
	// Redirect if url has "code" parameter in query string
	if(isset($_GET['code'])){
		header('location: ./');
	}
	
	// Retrieving user info from facebook
	try {
	$graphResponse = $fb->get('/me?fields=name,first_name,last_name,email, link,gender,picture');
	$fbUser = $graphResponse->getGraphicUser();
	} catch(FacebookResponseException $e) {
		echo 'Error occurred while retrieving user information : ' . $e->getMessage();
		session_destroy();
		header("Location: ./");
		exit;
	}
...
<html lang="en-US">
<head>
<title>SSD Assignment 2 ( Vithanage S A - IT17022248 )</title>
<meta charset="utf-8">
<!-- stylesheet file -->
